...
export async function loadExpense(token, params = {}) {
    ...
    const url = `https://hmcsapi.texva.id/api/expense?limit=100&firstday=${firstDay}&lastday=${lastDay}`;
...
export async function insertExpense(token, data) {
    // Add Expanse
    const url = 'https://hmcsapi.texva.id/api/expense';
    const dataPost = {
        date: data.date,
...
export async function updateExpense(token, idExpense, data) {
    // Edit Expanse
    const url = `https://hmcsapi.texva.id/api/expense/${idExpense}`;
    const response = await fetch(url, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
...
export async function getTypeExpense(token) {
    const url = 'https://hmcsapi.texva.id/api/tipe_expense';
    const response = await fetch(url, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
