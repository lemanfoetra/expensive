...
export async function loadIncome(token, params = {}) {
    ...
    const url = `https://hmcsapi.texva.id/api/income?limit=100&firstday=${firstDay}&lastday=${lastDay}`;
...
export async function insertIncome(token, data) {
    // Add Expanse
    const url = 'https://hmcsapi.texva.id/api/income';
    const response = await fetch(url, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
...
export async function updateIncome(token, id_income, data) {
    // Edit Expanse
    const url = `https://hmcsapi.texva.id/api/income/${id_income}`;
    const response = await fetch(url, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
